...
// Implement the main function of a torque actuator. applies a torque to the MatterSubSystem
...
int main() {
	...
	/// create torque actuators
	//OpenSim::TorqueActuator* Tq_ExoL;
	//OpenSim::TorqueActuator* Tq_ExoR;
	...
	/// we assume zero accelerations here for this example
	Vector QsUs(NX + 4); /// joint positions (Qs) and velocities (Us) - states
	for (int i = 0; i < NX; i++) QsUs[i] = 0;
...
/// set the pelvis x position to a specific value
	QsUs[1] = 2;

	/// set sate variables and realize
	...
	// Here we want to add a torque to the tibia and to the talus
	/// Note that these torques should be applied around the axis of the ankle joint
	/// Note, we re-implemented a torque actuator function here, because we the actuator class is not yet compatible
	/// with the current source code for automatic differentation.
	Vec3 ExoTorque(0,0,10);
	ExoActuation(*model, *tibia_l, *calcn_l, *state, ExoTorque, appliedBodyForces);
	ExoActuation(*model, *tibia_r, *calcn_r, *state, ExoTorque, appliedBodyForces);

	///
	///Vec3 ExoTorque = Vec3(0, 0, 10);
	///SimTK::MobilizedBodyIndex IndexTibiaL = tibia_l->getMobilizedBodyIndex();
	///SimTK::MobilizedBody Body_TibiaL = tibia_l->getMobilizedBody();
	///Vec3 ExoTiba_Ground = Body_TibiaL.expressVectorInGroundFrame(*state, ExoTorque);
	/// _bodyA->expressVectorInGround(s, torque);
	///model->getMatterSubsystem().addInBodyTorque(*state, IndexTibiaL, ExoTorque, appliedBodyForces);
	...
	// Test computation COP using forces expressed in ground

	Vector_<SpatialVec> groundForces(nbodies);
	groundForces.setToZero();

	// Update Vector of Spatial Vectors with GRF controls
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("calcn_l").getMobilizedBodyIndex(),locSphere_1_l, GRF_1_l[1], groundForces);
	SpatialVec GRF_Calcn_l = groundForces[model->getBodySet().get("calcn_l").getMobilizedBodyIndex()];
	Vec3 calcn_or_l = calcn_l->getPositionInGround(*state);
	Vec3 MomentGround = GRF_Calcn_l[0] + cross(calcn_or_l, GRF_Calcn_l[1]);
	std::cout << MomentGround << std::endl;
	...
	for (int i = 0; i < ndofr; ++i)	{
		res_os[i] = residualMobilityForces[indicesSimbodyInOS[i]];
	}

	// Try to acces locations in world
	///osim_double_adouble COPx1 = Force_values_1_l[3] / Force_values_1_l[1];
	///for (int i = 0; i < 12; ++i) {
	///	std::cout << Force_values_1_r[i] << std::endl;
	///}

	
	//std::cout << res_os << std::endl;
	//const auto svNames = model->getStateVariableNames();
	//std::cout << svNames << std::endl;

	//for (int i = 0; i < ndofr; ++i) {
	//	std::cout  << res_os.get(i) << std::endl;
	//}

	//std::cout << "Residual forces " << res_os << std::endl;
