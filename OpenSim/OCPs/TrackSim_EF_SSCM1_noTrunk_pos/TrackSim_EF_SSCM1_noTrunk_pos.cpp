...
int F_generic(const T** arg, T** res) {
    ...
    if (res[0]) {
        ...
        res[0][ndof+4*nc]     = value<T>(calcn_or_r[0]);  /// calcn_or_r_x
        res[0][ndof+4*nc + 1] = value<T>(calcn_or_r[2]);  /// calcn_or_r_z
        res[0][ndof+4*nc + 2] = value<T>(calcn_or_l[0]);  /// calcn_or_l_x
        res[0][ndof+4*nc + 3] = value<T>(calcn_or_l[2]);  /// calcn_or_l_x
        res[0][ndof+4*nc + 4] = value<T>(tibia_or_r[0]);  /// tibia_or_r_x
        res[0][ndof+4*nc + 5] = value<T>(tibia_or_r[2]);  /// tibia_or_r_z
        res[0][ndof+4*nc + 6] = value<T>(tibia_or_l[0]);  /// tibia_or_l_x
        res[0][ndof+4*nc + 7] = value<T>(tibia_or_l[2]);  /// tibia_or_l_z
    }
    return 0;
}
