...
int F_generic(const T** arg, T** res) {
    ...
    Vector knownUdot(NU);
    knownUdot.setToZero();
    for (int i = 0; i < 5; ++i) knownUdot[i] = u[i]; // joint accelerations
    // Order adjusted since order Simbody is different than order OpenSim
    knownUdot[5] = u[9]; // 5 Simbody is 9 OpenSim
    knownUdot[6] = u[5]; // 6 Simbody is 5 OpenSim
    knownUdot[7] = u[6]; // 7 Simbody is 6 OpenSim
    knownUdot[8] = u[7]; // 8 Simbody is 7 OpenSim
    knownUdot[9] = u[8]; // 9 Simbody is 8 OpenSim
