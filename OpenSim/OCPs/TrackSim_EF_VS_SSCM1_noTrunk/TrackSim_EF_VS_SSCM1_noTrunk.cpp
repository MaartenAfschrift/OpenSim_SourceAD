...
int F_generic(const T** arg, T** res) {
    ...
    knee_l = new CustomJoint("knee_l", *femur_l, Vec3(0.00460522, -0.38273715, -0.03429312), Vec3(0.0900458694776104, 0.0324588934006878, -0.0139927771155338),
        *tibia_l, Vec3(0), Vec3(0.105096325674, 0.048462535877, -0.005110024376), st_knee_l);
    knee_r = new CustomJoint("knee_r", *femur_r, Vec3(0.00150202, -0.38458563, -0.03710894), Vec3(0.0894230546547843, -0.138931558894303, -0.00323908703665436),
        *tibia_r, Vec3(0), Vec3(-0.064206849712, -0.022848068757, -0.001468892896), st_knee_r);
    ...
    back = new WeldJoint("back", *pelvis, Vec3(-0.072018, 0.029113, 0), Vec3(0), *torso, Vec3(0), Vec3(0));
    /// bodies and joints
    ...
    for (int i = 0; i < ndof; ++i) ua[i] = u[i];      // controls
