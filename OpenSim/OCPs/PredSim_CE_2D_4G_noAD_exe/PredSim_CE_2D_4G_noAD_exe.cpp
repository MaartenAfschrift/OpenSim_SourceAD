...
constexpr int ndof = 10;
constexpr int NX = 2*ndof;  // states
constexpr int NU = ndof;    // controls
constexpr int NR = 14;      // residual forces + GRFs. All in one output
...
int F_generic(const T** arg, T** res) {
    ...
    talus_l = new OpenSim::Body("talus_l", 0.0960558560811872, Vec3(0), Inertia(0.00110294603434762, 0.00110294603434762, 0.00110294603434762, 0, 0, 0));
    talus_r = new OpenSim::Body("talus_r", 0.0960558560811872, Vec3(0), Inertia(0.00110294603434762, 0.00110294603434762, 0.00110294603434762, 0, 0, 0));
    calcn_l = new OpenSim::Body("calcn_l", 1.20069820101484, Vec3(0.107156, 0.0321467, 0), Inertia(0.00154412444808667, 0.00430148953395572, 0.00452207874082525, 0, 0, 0));
    calcn_r = new OpenSim::Body("calcn_r", 1.20069820101484, Vec3(0.107156, 0.0321467, 0), Inertia(0.00154412444808667, 0.00430148953395572, 0.00452207874082525, 0, 0, 0));
    toes_l = new OpenSim::Body("toes_l", 0.208056984271851, Vec3(0.0370759, 0.00642934, 0.01875227), Inertia(0.000110294603434762, 0.000220589206869524, 0.000110294603434762, 0, 0, 0));
    toes_r = new OpenSim::Body("toes_r", 0.208056984271851, Vec3(0.0370759, 0.00642934, -0.0187522), Inertia(0.000110294603434762, 0.000220589206869524, 0.000110294603434762, 0, 0, 0));
    torso = new OpenSim::Body("torso", 32.8862592230917, Vec3(-0.0323155, 0.344699, 0), Inertia(1.64342095412182, 0.842051224712809, 1.59538335281789, 0, 0, 0));
    /// Joints - Transforms
    // Hip_l
    ...
    ankle_l = new PinJoint("ankle_l", *tibia_l, Vec3(0, -0.415562095018915, 0), Vec3(0), *talus_l, Vec3(0), Vec3(0));
    ankle_r = new PinJoint("ankle_r", *tibia_r, Vec3(0, -0.415562095018915, 0), Vec3(0), *talus_r, Vec3(0), Vec3(0));
    subtalar_l = new WeldJoint("subtalar_l", *talus_l, Vec3(-0.0522598265395693, -0.0449518089672941, -0.00848673008393251), Vec3(0), *calcn_l, Vec3(0), Vec3(0));
    subtalar_r = new WeldJoint("subtalar_r", *talus_r, Vec3(-0.0522598265395693, -0.0449518089672941, 0.00848673008393251), Vec3(0), *calcn_r, Vec3(0), Vec3(0));
    mtp_l = new WeldJoint("mtp_l", *calcn_l, Vec3(0.191594360985749, -0.00214311365755872, -0.00115728137508171), Vec3(0), *toes_l, Vec3(0), Vec3(0));
    mtp_r = new WeldJoint("mtp_r", *calcn_r, Vec3(0.191594360985749, -0.00214311365755872, 0.00115728137508171), Vec3(0), *toes_r, Vec3(0), Vec3(0));
    back = new PinJoint("back", *pelvis, Vec3(-0.0920471773757891, 0.0744969707659068, 0), Vec3(0), *torso, Vec3(0), Vec3(0));
    /// bodies and joints
    ...
    for (int i = 0; i < 5; ++i) ua[i] = u[i];      // controls
    ua[5] = u[9]; // 5 Simbody is 9 OpenSim
    ua[6] = u[5]; // 6 Simbody is 5 OpenSim
    ua[7] = u[6]; // 7 Simbody is 6 OpenSim
    ua[8] = u[7]; // 8 Simbody is 7 OpenSim
    ua[9] = u[8]; // 9 Simbody is 8 OpenSim
    ...
    appliedBodyForces[nfoot_r] = appliedBodyForces[nfoot_r] + GRF_heel_r + GRF_front2_r;
    /// left
    ...
    appliedBodyForces[nfoot_l] = appliedBodyForces[nfoot_l] + GRF_heel_l + GRF_front2_l;
    /// knownUdot
    ...
    SpatialVec GRF_r = GRF_heel_r + GRF_front2_r;
    SpatialVec GRF_l = GRF_heel_l + GRF_front2_l;
    ...
    if (res[0]) {
        ...
        res[0][5] = value<T>(residualMobilityForces[6]); // order adjusted since order Simbody is different than order OpenSim
        res[0][6] = value<T>(residualMobilityForces[7]);
        res[0][7] = value<T>(residualMobilityForces[8]);
        res[0][8] = value<T>(residualMobilityForces[9]);
        res[0][9] = value<T>(residualMobilityForces[5]);
        for (int i = 0; i < 2; ++i) {
            res[0][i + ndof] = value<T>(GRF_r[1][i]); // GRF_r
        }
        for (int i = 0; i < 2; ++i) {
            res[0][i + ndof + 2] = value<T>(GRF_l[1][i]); // GRF_l
        }
    }
    return 0;
}
